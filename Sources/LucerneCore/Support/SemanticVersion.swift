...
/// (`1.2.0-beta < 1.2.0`). Pre-release identifiers compare per SemVer §11:
...
public struct SemanticVersion: Comparable, Equatable, CustomStringConvertible {
    ...
    /// SemVer §11 pre-release ordering: compare dot-separated identifiers left to
    /// right — numeric ones numerically, numeric below alphanumeric, alphanumeric
    /// ones lexically — and, with all shared identifiers equal, fewer identifiers
    /// sorts lower (`alpha < alpha.1`).
    private static func prereleaseIsLower(_ lhs: String, than rhs: String) -> Bool {
        ...
        for (l, r) in zip(left, right) where l != r {
            switch (UInt(l), UInt(r)) {
            case let (ln?, rn?): return ln < rn   // both numeric: 2 < 10
            case (_?, nil):      return true      // numeric < alphanumeric
            case (nil, _?):      return false
            case (nil, nil):     return l < r     // both alphanumeric: lexical
            }
        }
